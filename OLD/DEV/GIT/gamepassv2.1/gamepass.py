...
#
# Implemented:
# - SIGL list (catalog.gamepass.com)
# - Store product metadata (DisplayCatalog) with autosplit + cache + resume
# - Wikidata enrichment (Genre, Year, Description, Wikipedia link, Review score P444)
# - Progress indication: tqdm if available; otherwise informative heartbeat with phase + counters + ETA
# - HTML UI: view switch (Table/Tiles), filters (Years/Genres/Rating), Favorites mode, per-row description toggle
#
# Changes requested:
# 1) Platforms handling REMOVED entirely.
# 2) Description column moved right after Genre.
# 3) Rating column fixed width + ellipsis; in TABLE it shows ONLY the numeric rating (0-100).
#    RatingSource kept in data/CSV and shown as tooltip (title) where applicable.
#
# Optional:
# - Hours (HowLongToBeat) via howlongtobeatpy (enable with --hltb)
#
# Usage:
#   chmod +x gamepass.py
#   ./gamepass.py
...
# ---------------- Informative progress / heartbeat ----------------
_last_heartbeat = 0.0
PHASE = "idle"
PHASE_DONE = 0
PHASE_TOTAL = 0
...
def heartbeat(force: bool = False):
    ...
    now = time.time()
    if not (force or (now - _last_heartbeat) >= 5.0):
        return
    _last_heartbeat = now
...
# ---------------- Wikidata enrichment (NO platforms) ----------------
...
def wikidata_enrich(qid: str) -> dict:
    ...
    for r in rows:
        ...
        candidates.append((g, d, w, y, s, rv))

    for (g, d, w, y, s, rv) in candidates:
        if not genres and g:
            genres = g
        if not desc and d:
            desc = d
    ...
    metacritic = [c for c in candidates if (c[5] or "").lower().find("metacritic") >= 0 and c[4]]
    if metacritic:
        score_best = metacritic[0][4]
        reviewer_best = metacritic[0][5] or "Metacritic"
    else:
        anyscore = [c for c in candidates if c[4]]
        if anyscore:
            score_best = anyscore[0][4]
            reviewer_best = anyscore[0][5] or "Review score"
...
<head>
  ...
  <style>
    ...
    .col-rating { width:64px; max-width:64px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; text-align:center; font-weight:600; }
...
<body>
  <h1>__TITLE__</h1>
  <div class="meta">
    Вигляд: таблиця / плитки. Фільтри: рік, жанр, рейтинг. Обране ⭐ зберігається у localStorage.
  </div>
...
function renderTable(list){
  ...
  tb.innerHTML = list.map((x, idx)=>{
    ...
    // Rating: number only in table. Source via tooltip (title).
...
def save_outputs(rows: List[dict]):
    ...
    # CSV (platforms removed)
    cols = ["id","name","genre","rating","ratingSource","year","hours","publisher","developer","description","wikipedia","wikidata","image"]
...
def main():
    ...
    print(f"Total ids: {len(ids)} | Cached products: {cached_ok} | To fetch: {len(to_fetch)}")

    set_phase("store", total=len(to_fetch))
    pbar = tqdm(total=len(to_fetch), desc="Store fetch", unit="item") if (tqdm and len(to_fetch)>0) else None
    ...
    if enrich_start > 0 and enrich_start < len(ids):
        print(f"Resuming enrichment from index: {enrich_start}/{len(ids)}")

    set_phase("enrich", total=max(0, len(ids) - enrich_start))
    ebar = tqdm(total=max(0, len(ids) - enrich_start), desc="Enrichment", unit="game") if (tqdm and enrich_start < len(ids)) else None
    ...
    for idx in range(enrich_start, len(ids)):
        ...
        # Wikidata
        wd = {"Genres_WD":"","Description_WD":"","Year_WD":"","Rating":"","RatingSource":"","WikipediaUrl":"","WikidataUrl":""}
